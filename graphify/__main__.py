...
from graphify.paths import graphify_out_dir, graphify_out_for_watch, default_graph_json_path, graphify_out_rel
...
def uninstall_all(project_dir: Path | None = None, purge: bool = False) -> None:
    ...
    if purge:
        ...
        out = pd / graphify_out_rel()
        if out.exists():
            _shutil.rmtree(out)
            print(f"\n  {graphify_out_rel()}/  ->  deleted (--purge)")
        else:
            print(f"\n  {graphify_out_rel()}/  ->  not found (nothing to purge)")
...
def main() -> None:
    ...
    if cmd == "install":
        ...
        p.add_argument("--memory-dir", default=str(graphify_out_dir() / "memory"))

...
def _is_call_edge(data: dict) -> bool:
    rel = str(data.get("relation") or "").strip().lower()
    if rel in _CALL_RELATIONS:
        return True
    return str(data.get("context") or "").strip().lower() == "call"


def _extract_callers_target(question: str) -> str | None:
...
def _direct_callers_text(G: nx.Graph, label: str, *, limit: int = 200) -> str:

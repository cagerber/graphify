...
# Enrich-generated folder links. ``graphify.enrich`` adds them to graph.json after
# every build (``GRAPHIFY_FOLDER_EDGES``), so they are derived, regenerated data —
# not code topology.
_DERIVED_FOLDER_RELATIONS = frozenset({"same_directory", "shared_folder"})


def _topology_for_compare(graph_data: dict) -> dict:
    """``_canonical_topology_for_compare`` minus enrich-generated folder links.

    The folder links are assigned to the alphabetically-first file hub of a group
    and are indistinguishable from an authored edge to the graph builders: on an
    undirected graph the pair one of them occupies can already hold an import edge,
    so which of the two survives depends on write order — an incremental rebuild
    (enriched graph preserved + merged) and a full rebuild (fresh extraction, enrich
    still to run) legitimately end up with different folder-link sets for identical
    code. Comparing them made ``_rebuild_code`` report a phantom topology change,
    which skipped the unchanged-topology repair of a missing ``graph.html`` and fell
    through to a full re-cluster (tests/test_watch.py::…viz_cap). Derived links are
    regenerated by the next enrich pass, so they carry no signal here.
    """
    topology = _canonical_topology_for_compare(graph_data)
    for key in ("links", "edges"):
        ...
        topology[key] = [
            ...
            if not (
                ...
                and edge.get("relation") in _DERIVED_FOLDER_RELATIONS
            )
        ]
    return topology
    ...
    *,
    ...
    try:
        ...
        if existing_graph_data:
            try:
                same_topology = (
                    json.dumps(_topology_for_compare(existing_graph_data), sort_keys=True, ensure_ascii=False)
                    == json.dumps(_topology_for_compare(candidate_topology), sort_keys=True, ensure_ascii=False)
                )
            except Exception:
                same_topology = False
            if same_topology:

...
def test_unmatched_consumer_suffix_never_falls_back_to_another_language(
    ...
    Upstream maps ``.cls`` to the Apex extractor, which emits wrong-language nodes
    for a file the consumer has declared as its own.
    """
    root = tmp_path
    _write_fake_consumer(root)
    (root / "pyproject.toml").write_text(
...
""".strip()
    ...
    assert extractors == [extract_consumer_owned]
